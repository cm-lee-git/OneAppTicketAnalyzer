...
if "%SKIP_SELF%"=="0" schtasks /delete /TN "CCI_AutoUpdate"      /F 2>nul

:: CCI 실행 작업 등록 (마스터 페이지 + 스냅샷 구조)
schtasks /create /TN "CCI_Doc1"           /TR "%DIR%\run_doc1.bat"     /SC WEEKLY /D "MON,TUE,WED,THU,FRI" /ST 10:00 /F
...
if "%SKIP_SELF%"=="0" schtasks /create /TN "CCI_AutoUpdate"     /TR "%DIR%\update.bat"       /SC WEEKLY /D "MON,TUE,WED,THU,FRI" /ST 09:00 /F

:: 절전 해제 후 실행 + 예약 시각 놓쳤을 때 켜지면 즉시 실행 + 배터리 상태에서도 실행
:: schtasks는 미지원이므로 PowerShell로 사후 적용
powershell -NoProfile -Command ^
  "Get-ScheduledTask -TaskName 'CCI_*' | ForEach-Object { $s = $_.Settings; $s.WakeToRun = $true; $s.StartWhenAvailable = $true; $s.DisallowStartIfOnBatteries = $false; $s.StopIfGoingOnBatteries = $false; Set-ScheduledTask -TaskName $_.TaskName -Settings $s } | Out-Null"
